...
Description:
    Orchestrates the Transformation and Loading (ETL) process from the 'bronze'
    layer to the 'silver' layer.
Script Purpose:
    - Cleanses, standardizes, and validates data from Bronze tables.
    - Handles data type conversions (Strings to Dates/Numbers).
    - Implements business logic (Categorization, Mapping, Deduplication).
    - Ensures idempotency by truncating silver tables before each load.
    - Provides execution telemetry (duration per table and total batch time).
...
BEGIN
    ...
    -- ----------------SECTION: silver.crm_cust_info----------------

    RAISE NOTICE '>> Loading silver.crm_cust_info';

    -- Record the start time of the silver.crm_cust_info
    ...
    RAISE NOTICE 'Load Duration of silver.crm_cust_info: % seconds', v_duration;
    RAISE NOTICE '>> -------------';


    -- ----------------SECTION: silver.crm_prd_info----------------

    RAISE NOTICE '>> Loading silver.crm_prd_info';

    -- Record the start time of silver.crm_prd_info
    v_start_time := CLOCK_TIMESTAMP();
    ...
    v_end_time := CLOCK_TIMESTAMP();
    v_duration := EXTRACT(EPOCH FROM (v_end_time - v_start_time));
    RAISE NOTICE 'Load Duration of silver.crm_prd_info: % seconds', v_duration;
    RAISE NOTICE '>> -------------';


    -- ----------------SECTION: silver.crm_sales_details----------------

    RAISE NOTICE '>> Loading silver.crm_sales_details';

    -- Record the start time of silver.crm_sales_details
    v_start_time := CLOCK_TIMESTAMP();
    ...
    v_end_time := CLOCK_TIMESTAMP();
    v_duration := EXTRACT(EPOCH FROM (v_end_time - v_start_time));
    RAISE NOTICE 'Load Duration of silver.crm_sales_details: % seconds', v_duration;
    RAISE NOTICE '>> -------------';


    -- ----------------SECTION: silver.erp_cust_az12----------------

    RAISE NOTICE '>> Loading silver.erp_cust_az12';

    -- Record the start time of silver.erp_cust_az12
    v_start_time := CLOCK_TIMESTAMP();
    ...
    v_end_time := CLOCK_TIMESTAMP();
    v_duration := EXTRACT(EPOCH FROM (v_end_time - v_start_time));
    RAISE NOTICE 'Load Duration of silver.erp_cust_az12: % seconds', v_duration;
    RAISE NOTICE '>> -------------';


    -- ----------------SECTION: silver.erp_loc_a101----------------

    RAISE NOTICE '>> Loading silver.erp_loc_a101';

    -- Record the start time of silver.erp_loc_a101
    v_start_time := CLOCK_TIMESTAMP();
    ...
    v_end_time := CLOCK_TIMESTAMP();
    v_duration := EXTRACT(EPOCH FROM (v_end_time - v_start_time));
    RAISE NOTICE 'Load Duration of silver.erp_loc_a101: % seconds', v_duration;
    RAISE NOTICE '>> -------------';


    -- ----------------SECTION: silver.erp_px_cat_g1v2----------------

    RAISE NOTICE '>> Loading silver.erp_px_cat_g1v2';

    -- Record the start time of erp_px_cat_g1v2
    v_start_time := CLOCK_TIMESTAMP();
    ...
    v_end_time := CLOCK_TIMESTAMP();
    v_duration := EXTRACT(EPOCH FROM (v_end_time - v_start_time));
    RAISE NOTICE 'Load Duration of silver.erp_px_cat_g1v2: % seconds', v_duration;
    RAISE NOTICE '>> -------------';
